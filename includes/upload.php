...
class Media {
        ...
        if ($this->process()) {
            ...
            if ($this->user_image_destroy($id)) {
                if (move_uploaded_file($this->fileTempPath, $this->userPath . '/' . $this->fileName)) {
                    return $this->update_user_img($id);
                } else {
                    $this->errors[] = 'The file upload failed, possibly due to incorrect permissions on the upload folder.';
                }
            }
    ...
    private function update_user_img($id) {
        global $db;
    ...
    private function insert_media() {
        ...
        $sql = "INSERT INTO media (file_name, file_type) VALUES ('{$db->escape($this->fileName)}', '{$db->escape($this->fileType)}')";
        return $db->query($sql);
    }
    ...
    public function media_destroy($id, $file_name) {
        ...
        if (delete_by_id('media', $id)) {
            ...
            $this->errors[] = 'Photo deletion failed or missing parameters.';
            return false;
        }
    }
